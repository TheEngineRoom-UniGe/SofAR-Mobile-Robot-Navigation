...
public class JointControl : MonoBehaviour
{
    ...
    void FixedUpdate(){
        ...
        if(joint.jointType != ArticulationJointType.FixedJoint)
        {
            ...
            else if (controltype == RosSharp.Control.ControlType.VelocityControl)
            {
                ArticulationDrive currentDrive = joint.xDrive;
                float newTargetDelta = (int)direction * Time.fixedDeltaTime * acceleration;
                currentDrive.targetVelocity += newTargetDelta;
                joint.xDrive = currentDrive;
            }
